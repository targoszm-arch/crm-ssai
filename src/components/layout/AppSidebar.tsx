...
import { Users, ShoppingCart, LineChart, Calendar, CreditCard, Settings, Home, MessageSquare, GanttChartSquare, Wallet, Mail, Workflow, DollarSign } from "lucide-react";
      ...
      <SidebarHeader className="border-b border-sidebar-border h-14 flex items-center px-4">
        <Link to="/" className="flex items-center gap-2 font-semibold">
          <div className="flex items-center justify-center h-6 w-6 rounded-md bg-primary shrink-0">
            <span className="text-primary-foreground text-sm font-bold">SS</span>
          </div>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {mainNavItems.map(item => <SidebarMenuItem key={item.path}>
                  <SidebarMenuButton asChild isActive={currentPath === item.path} tooltip={item.label}>
                    <Link to={item.path}>
