...
import { Linkedin, ExternalLink, X, User, Link2, Sparkles, Loader2, Copy, Check, UserPlus, PlusCircle } from "lucide-react";
...
export function LinkedInMessageView({ message, onClose }: LinkedInMessageViewProps) {
  ...
  // Get headline from connection
  const headline = message.connection?.headline || "";

  // Parse name into first/last for prefilling
  ...
  const handleContactCreated = async (contactId: string) => {
    ...
    toast({
      ...
      description: `${firstName} ${lastName} has been added to your CRM and linked.`,
    });
  };
