  ...
  return <div><PageShell>
    ...
    <div className="grid items-start gap-4 lg:grid-cols-[minmax(0,1.5fr)_minmax(300px,.9fr)]">
      <div className="space-y-4">
        <Card><CardHeader className="border-b"><CardTitle className="text-base">Company summary</CardTitle></CardHeader><CardContent className="space-y-5 p-5"><p className="text-sm text-muted-foreground">{company.description || "No company summary added yet."}</p><div className="grid gap-4 sm:grid-cols-2"><Field label="Industry" value={company.industry} /><Field label="Sales Stage" value={company.stage} /><Field label="Location" value={company.country} /><Field label="Company Size" value={company.employee_range} /><Field label="NAICS code" value={company.naics_code ? `${company.naics_code} — ${company.naics_description}` : null} /><Field label="SIC code" value={company.sic_code ? `${company.sic_code} — ${company.sic_title}` : null} /></div><div className="grid gap-4 sm:grid-cols-2"><LinkField label="Website" href={(() => { const site = company.website || company.domains; return site ? (site.startsWith("http") ? site : `https://${site}`) : null; })()} text={company.website || company.domains} /><LinkField label="LinkedIn" href={company.linkedin_url} text={company.linkedin_url ? "Open profile" : null} /></div></CardContent></Card>
