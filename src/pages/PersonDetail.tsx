  ...
  return <div className="min-h-full bg-muted/20 p-5 md:p-7">
    <div className="w-full">
      <header className="mb-4 flex flex-wrap items-center justify-between gap-3 border-b pb-4">
        ...
        <div className="flex gap-2"><Button variant="outline" size="sm" asChild><a href={contact.email ? `mailto:${contact.email}` : undefined}><Mail className="mr-2 h-4 w-4" />Send email</a></Button>{companyName && contact.company_id && <Button variant="outline" size="sm" asChild><Link to={`/companies/${contact.company_id}?from=customers`}><Building2 className="mr-2 h-4 w-4" />Open company</Link></Button>}<Button size="sm" onClick={() => setActiveTab("activity")}><Sparkles className="mr-2 h-4 w-4" />Log activity</Button></div>
      </header>
